...
4) Amount of columns are not the same (There are 14 in messy and 12 in original )
5) Data types are incorrect (In messy are character while original are numeric (excpet for 'model' column))
6) 
...
all.equal(clean.mtcars.datatype, goal.mtcars)
```

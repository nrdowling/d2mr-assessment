...
#Add to final
finalclean.matcars.99 <- theclean.mtcars.spaces

#Check for spaces
head(finalclean.matcars.99)
```
...
#cyl impute
clean.mtcars.impute.cyl.hp <- finalclean.matcars %>%
  ungroup() %>%
  mutate(
    cyl = case_when(
...
finalclean.matcars.1
#Add to final 
finalclean.matcars.1 <- clean.mtcars.impute.cyl.hp
all.equal(finalclean.matcars.1, goal.mtcars)
```
...
clean.mtcars.round <- finalclean.matcars.1 %>%
  mutate(
    disp = round(disp, 0),
    drat = round(drat, 2),
    qsec = round(qsec, 2)
...
#Add to final
finalclean.matcars.2 <- clean.mtcars.round
all_equal(finalclean.matcars.2, goal.mtcars)
finalclean.matcars.2
```
...
clean.mtcars.typos <- finalclean.matcars.2 %>%
  mutate(
    model = str_replace_all(model, "Hornet SportAbout", "Hornet sportabout"))

#Check for typos
finalclean.matcars.3 <- clean.mtcars.typos
finalclean.matcars.3
...
clean.mtcars.rmvecol <- finalclean.matcars.3 %>%
  select(-13, -14) %>%
  select (, 1:12)
...
#Add to final
finalclean.matcars.4 <- clean.mtcars.rmvecol

finalclean.matcars.4
...
# all.equal()
all.equal(finalclean.matcars.4, goal.mtcars)
# daff
library(daff)
diff_data(finalclean.matcars.4, goal.mtcars)
render_diff(diff_data(finalclean.matcars.4, goal.mtcars))
finalclean.matcars.4
```

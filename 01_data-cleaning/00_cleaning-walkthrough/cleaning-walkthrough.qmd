---
title: 'Data Cleaning: Walkthrough (iris)'
---
...
- Columns are out of order TRUE
- Column names don't match TRUE
- Data types are incorrect (all are character in messy, but numeric/factor in original) TRUE
- Values in the species column have lots of problems (extra spaces, capitalization, typos, etc.) TRUE
- Numbers have commas in them instead of periods TRUE
- Numbers aren't rounded to the same number of places TRUE

2. What are the steps you need to take to clean the messy dataset? 

- Rename columns TRUE 
- Reorder columns TRUE
- Change data types: Species to factor, other columns to numeric TRUE
- Correct values in Species column: remove extra spaces, fix typos, ensure consistent capitalization TRUE
- Remove commas from numbers TRUE
- Round numbers to the same number of decimal places (1) TRUE
